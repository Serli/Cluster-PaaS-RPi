...
var Gossiper = exports.Gossiper = function (port, seeds) {
...
Gossiper.prototype.start = function (callback) {
    ...
    this.server = net.createServer(function (net_stream) {
        ...
        mp_stream.on('msg', function (msg) {
            self.handleMessage(net_stream, mp_stream, msg)
        });
    });

    this.peer_name = [local_ip, this.port].join(':');
    this.peers[this.peer_name] = this.my_state;
    this.my_state.name = this.peer_name;
    this.server.listen(this.port, local_ip, callback);
...
Gossiper.prototype.listenToPeer = function (peer) {
    ...
    peer.on('peer_alive', function (threshold, phi) {
        self.emit('peer_alive', {ip : peer.name, threshold : threshold, phi : phi});
    });
    peer.on('peer_failed', function (threshold, phi) {
        self.emit('peer_failed', {ip : peer.name, threshold : threshold, phi : phi});
    });
};
